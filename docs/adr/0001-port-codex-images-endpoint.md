...
- Headers:
  - Auth is `Authorization: Bearer {token}` plus `ChatGPT-Account-ID` ([bearer_auth_provider.rs#L32-L46](https://github.com/openai/codex/blob/c77c34ed33877a6e5b3759703d01d3b223274cbf/codex-rs/model-provider/src/bearer_auth_provider.rs#L32-L46)).
  - The image backend adds `x-codex-image-turn-id: {turn_id}` and the thread `originator` ([backend.rs#L113-L122](https://github.com/openai/codex/blob/c77c34ed33877a6e5b3759703d01d3b223274cbf/codex-rs/ext/image-generation/src/backend.rs#L113-L122)).
  - The default client adds `User-Agent` and `originator: codex_cli_rs` ([default_client.rs#L40](https://github.com/openai/codex/blob/c77c34ed33877a6e5b3759703d01d3b223274cbf/codex-rs/login/src/auth/default_client.rs#L40), [default_client.rs#L137-L151](https://github.com/openai/codex/blob/c77c34ed33877a6e5b3759703d01d3b223274cbf/codex-rs/login/src/auth/default_client.rs#L137-L151)).
- Retry:
  - A request is retried up to `DEFAULT_REQUEST_MAX_RETRIES = 4` times, so 5 attempts in total ([lib.rs#L31](https://github.com/openai/codex/blob/c77c34ed33877a6e5b3759703d01d3b223274cbf/codex-rs/model-provider-info/src/lib.rs#L31), [retry.rs#L80-L107](https://github.com/openai/codex/blob/c77c34ed33877a6e5b3759703d01d3b223274cbf/codex-rs/codex-client/src/retry.rs#L80-L107)).
  - The backoff starts at 200 ms and doubles per attempt, with a random jitter factor between 0.9 and 1.1 ([lib.rs#L371-L377](https://github.com/openai/codex/blob/c77c34ed33877a6e5b3759703d01d3b223274cbf/codex-rs/model-provider-info/src/lib.rs#L371-L377), [retry.rs#L39-L48](https://github.com/openai/codex/blob/c77c34ed33877a6e5b3759703d01d3b223274cbf/codex-rs/codex-client/src/retry.rs#L39-L48)).
  - Only 5xx responses and transport errors are retried. 429 is not retried ([retry.rs#L22-L37](https://github.com/openai/codex/blob/c77c34ed33877a6e5b3759703d01d3b223274cbf/codex-rs/codex-client/src/retry.rs#L22-L37)).
  - The response body is read inside the retried closure, so a connection dropped mid-body is retried as a transport error ([transport.rs#L114-L137](https://github.com/openai/codex/blob/c77c34ed33877a6e5b3759703d01d3b223274cbf/codex-rs/http-client/src/transport.rs#L114-L137)).
- Response:
  - The body is JSON `{created, data: [{b64_json, generation_id?}], background?, quality?, size?}` ([images.rs#L55-L72](https://github.com/openai/codex/blob/c77c34ed33877a6e5b3759703d01d3b223274cbf/codex-rs/codex-api/src/images.rs#L55-L72)).
  - The first `data` element is used. An empty array is reported as "image generation returned no image data" ([tool.rs#L185-L197](https://github.com/openai/codex/blob/c77c34ed33877a6e5b3759703d01d3b223274cbf/codex-rs/ext/image-generation/src/tool.rs#L185-L197)).
...
- Errors:
  - A non-2xx response becomes `TransportError::Http`, rendered as `http {status}: {body:?}` ([transport.rs#L114-L131](https://github.com/openai/codex/blob/c77c34ed33877a6e5b3759703d01d3b223274cbf/codex-rs/http-client/src/transport.rs#L114-L131), [error.rs#L10](https://github.com/openai/codex/blob/c77c34ed33877a6e5b3759703d01d3b223274cbf/codex-rs/http-client/src/error.rs#L10)).
  - Undecodable JSON becomes `stream error: failed to decode image generation response: ...` ([images.rs#L77-L78](https://github.com/openai/codex/blob/c77c34ed33877a6e5b3759703d01d3b223274cbf/codex-rs/codex-api/src/endpoint/images.rs#L77-L78)).
  - The tool wraps every failure as `image generation failed: {message}` ([tool.rs#L172-L177](https://github.com/openai/codex/blob/c77c34ed33877a6e5b3759703d01d3b223274cbf/codex-rs/ext/image-generation/src/tool.rs#L172-L177)).
- Model-facing args: the tool takes `prompt`, `referenced_image_paths` (at most 5 paths) and `num_last_images_to_include` (1 to 5). The two reference selectors are mutually exclusive ([tool.rs#L86-L95](https://github.com/openai/codex/blob/c77c34ed33877a6e5b3759703d01d3b223274cbf/codex-rs/ext/image-generation/src/tool.rs#L86-L95), [tool.rs#L419-L488](https://github.com/openai/codex/blob/c77c34ed33877a6e5b3759703d01d3b223274cbf/codex-rs/ext/image-generation/src/tool.rs#L419-L488), [imagegen_description.md](https://github.com/openai/codex/blob/c77c34ed33877a6e5b3759703d01d3b223274cbf/codex-rs/ext/image-generation/imagegen_description.md)).
...
4. Keep the plugin's own tool contract where it is not part of the request mirror. `out` is the save location. `size` is restated in the prompt because that is the only working dimension control. `images` keeps its current name and is capped at codex's `MAX_EDIT_IMAGES = 5`.
5. Parse the tool arguments once with the zod schema at the start of `execute`. OpenCode builds the model-facing JSON schema from the zod args but passes the model's arguments to `execute` without validating them. This was verified on OpenCode 1.18.29: `packages/opencode/src/tool/registry.ts` (`fromPlugin`) builds a `safeParse`-based `parameters` guard that is never applied to plugin tools, and `packages/opencode/src/session/tools.ts` wraps the tool with `jsonSchema(schema)` without a validator before calling `execute`; plugin tools also bypass the `wrap()` in `packages/opencode/src/tool/tool.ts` where built-in tools get their parameters decoded. Without the parse in `execute`, the image cap and the size grammar would be hints only.
6. Drop the `quality` argument: the backend ignores it and codex deliberately sends `auto`. A caller that still passes it is not rejected, because the parse in decision 5 strips unknown keys; the value is simply not read.
7. Do not port `num_last_images_to_include`. Codex resolves it from the conversation history, which the OpenCode plugin API does not expose. This was decided by the maintainer for this ADR.
8. Do not bump to a gpt-image-2.5 id (see above).
9. Remove the `eventsource-parser` dependency; the endpoint returns plain JSON.
10. Make the versioned-save message prescriptive, in the spirit of codex's output hint: it names the pre-existing file as untouched and forbids overwriting or renaming it. Observed in e2e runs: without this, agents `mv` the versioned file onto the requested path, defeating the non-overwrite guarantee. Codex's literal wording ("copy it and leave the original in place") is not reused because in this plugin "the original" would be read as the versioned file and "another path" as the requested one.
...
| 2 | `src/codex.ts` (`withSizeNote`) | Appends `Output image size — width/height` to the prompt when `size` is `WIDTHxHEIGHT` | Codex has no size arg; its model writes dimensions into the prompt itself. The backend ignores the structured field. On `images/generations` it honors prompt dimensions exactly, verified 2026-07-13 and 2026-09-10. On `images/edits` it keeps only the aspect ratio, observed 2026-09-10. The tool description states both |
...
| 8 | `src/codex.ts` (retry) | Every fetch rejection is treated as a retryable transport error; codex retries only its `Timeout`/`Connection`/`Network` variants. Header values are validated once before the loop so an invalid token fails fast, with a fixed message because the runtime's own message echoes the header value | The error classes fetch rejects with are not consistent across runtimes (the spec and Node use `TypeError` for network failures, Bun uses `Error` with a `code`), so no class-based classification is portable. Validating the headers up front is the one construction error that can be told apart everywhere |
...
| 10 | `src/input-image.ts` | Paths may be relative and are resolved against the OpenCode context dir. MIME comes from magic-byte sniffing via `file-type`. Nothing is re-encoded | Codex requires absolute paths and re-encodes non-PNG/JPEG/WebP inputs to PNG. A raster re-encoder is a heavy dependency for a plugin. Known gap: GIF references are sent unconverted |
