...
const NO_DELAY = { ...TURN, retryBaseDelayMs: 0 }
...
describe("callViaCodexImages", () => {
  ...
  test("posts a generations request and returns the first image's base64", async () => {
    ...
    expect(headers["x-codex-image-turn-id"]).toBe("msg_1")
  ...
  test("keeps the prompt unchanged when size is omitted", async () => {
    ...
    await callViaCodexImages(AUTH, ARGS, [], TURN)
  ...
  test("keeps the prompt unchanged when size is auto", async () => {
    ...
    await callViaCodexImages(AUTH, { ...ARGS, size: "auto" }, [], TURN)
  ...
  test("treats a data element without b64_json as a decode failure", async () => {
  ...
  test("does not retry a 429 response", async () => {
    ...
    await expect(callViaCodexImages(AUTH, ARGS, [], TURN)).rejects.toThrow(
      "image generation failed: http 429: slow down",
    )
    expect(fetchMock).toHaveBeenCalledTimes(1)
  })
  ...
  test("propagates an abort raised during the retry backoff without exhausting the retries", async () => {
    ...
    const promise = callViaCodexImages(AUTH, ARGS, [], {
      ...TURN,
