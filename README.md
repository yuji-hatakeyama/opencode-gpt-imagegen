...
- **Reference images.** Pass up to 5 input images alongside the prompt for style guidance, edit targets, or compositing inputs.
...
Pass up to 5 image paths via the `images` argument and the model uses them as references for the next generation — for style guidance, characters to keep, scenes to extend, and so on.
